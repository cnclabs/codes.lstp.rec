...
class Recommender(object):
    ...
    def __init__(self,
        ...
        self._device = torch.device("cuda:1" if use_cuda else "cpu")
    ...
    def fit(self, train, test, verbose=False):
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            t2 = time()
            if verbose and (epoch_num + 1) % 100 == 0:
                precision, recall, ndcg, mean_aps = evaluate_ranking(self, test, train, k=[10, 20])
                output_str = "Epoch %d [%.1f s]\tloss=%.4f, map=%.4f, " \
                             "prec@10=%.4f, prec@20=%.4f, " \
                             "recall@10=%.4f, recall@20=%.4f, " \
                             "NDCG@10=%.4f, NDCG@20=%.4f [%.1f s]" % (epoch_num + 1,
                                                                                         t2 - t1,
                                                                                         epoch_loss,
                                                                                         mean_aps,
                                                                                         np.mean(precision[0]),
...
if __name__ == '__main__':
    ...
    # data arguments
    parser.add_argument('--train_root', type=str, default='datasets/beauty/test/train.txt')
    parser.add_argument('--test_root', type=str, default='datasets/beauty/test/test.txt')
